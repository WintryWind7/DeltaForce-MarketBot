...
单端价格刷新行为模块 - SPR000X
通过刷新机制检测低价子弹并自动购买
代码ID: SPR000X (Single Price Refresh)
"""
...
class SinglePriceRefreshBehavior(QThread):
    """单端价格刷新行为线程 - SPR000X"""
    ...
    def __init__(self, window_handles, config=None):
        ...
        try:
            if DeltaForceClass and window_handles:
                ...
                if not self.delta.bind_to_window(window_handles[0]):
                    self.log_message.emit("❌ [SPR000X] 窗口绑定失败")
                    self.delta = None
                else:
                    # 设置日志回调
                    self.delta.set_log_callback(self.log_message.emit)
                    self.log_message.emit(f"✅ [SPR000X] DeltaForce实例初始化成功")
                    ...
                    try:
                        ...
                        if torch.cuda.is_available():
                            ...
                            self.log_message.emit(f"🚀 [SPR000X] OCR使用GPU加速: {gpu_name}")
                        else:
                            self.log_message.emit(f"⚠️ [SPR000X] GPU不可用，OCR使用CPU模式")
                    except ImportError:
                        self.log_message.emit(f"⚠️ [SPR000X] 无法检测GPU状态（PyTorch未安装）")
            else:
                self.log_message.emit(f"❌ [SPR000X] DeltaForce实例初始化失败：缺少必要参数")
        except Exception as e:
            self.delta = None
            self.log_message.emit(f"❌ [SPR000X] DeltaForce实例初始化异常: {e}")
        ...
        self.log_message.emit(f"🎯 [SPR000X] 单端价格刷新行为初始化完成")
    ...
    def _create_log_file_path(self):
        ...
        try:
            ...
            filename = f"SPR000X_{timestamp}.csv"
    ...
    def recognize_price(self, debug=False):
        ...
        try:
            ...
            if debug:
                self.log_message.emit(f"🔍 [调试] 识别区域: ({screen_left},{screen_top}) 到 ({screen_right},{screen_bottom})")
            
            # 获取预处理后的图像（与test_300.py相同的方式）
            processed_image = self.delta.ocr._preprocess_image(
                self.delta.ocr._screenshot((screen_left, screen_top), (screen_right, screen_bottom)), 
                "peizhuang"
            )
            ...
            if debug:
                self.log_message.emit(f"🔍 [调试] EasyOCR原始结果: {raw_results}")
            
            # 合并文本（与test_300.py相同的方式）
            combined_text = "".join(text for _, text, _ in raw_results)
            
            if debug:
                self.log_message.emit(f"🔍 [调试] 合并文本: '{combined_text}'")
            ...
            if combined_text and combined_text.isdigit():
                ...
                if debug:
                    self.log_message.emit(f"🔍 [调试] 计算结果: 总价={total_price}, 单价={unit_price}")
                
                return total_price, unit_price
            else:
                if debug:
                    self.log_message.emit(f"🔍 [调试] 文本无效: '{combined_text}' (空白或非数字)")
                return None, None
            ...
            if debug:
                import traceback
                self.log_message.emit(f"🔍 [调试] 异常详情: {traceback.format_exc()}")
            return None, None
    ...
    def perform_purchase(self):
        ...
        try:
            ...
            # 移动到购买按钮位置
            button_screen = self.delta.ratio_to_screen_coords(purchase_button[0], purchase_button[1])
            pyautogui.moveTo(button_screen[0], button_screen[1])
            
            # 连续点击5次，每次间隔0.2秒
            for i in range(5):
                pyautogui.click(button_screen[0], button_screen[1])
                time.sleep(0.2)
    ...
    def run(self):
        ...
        try:
            self.log_message.emit("🚀 [SPR000X] 单端价格刷新行为开始运行")
            ...
            while not self.should_stop:
                try:
                    ...
                    # 识别价格（前3次或连续失败5次后启用调试模式）
                    debug_mode = self.debug_mode or self.refresh_count <= 3 or consecutive_failures >= 5
            ...
            self.log_message.emit("🏁 [SPR000X] 单端价格刷新行为已停止")

# 导出行为类
def get_behavior_class():
    """返回行为类"""
    return SinglePriceRefreshBehavior
