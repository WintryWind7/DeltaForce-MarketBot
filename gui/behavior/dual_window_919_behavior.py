...
BEHAVIOR_INFO = {
    "title": "双端满仓919",
    "description": "自动在主辅两个DeltaForce窗口之间切换聚焦状态，每隔2秒切换一次，适用于双开操作场景。",
    "version": "1.0.0",
...
class DualWindow919Behavior(QThread):
    """双端满仓919行为线程"""
    ...
    def stop_behavior(self):
        ...
        self.log_message.emit("🛑 正在停止双端满仓919...")
    ...
    def output_recognition_stats(self):
        ...
        self.log_message.emit("📊 识别准确率统计报告")
        self.log_message.emit("=" * 50)
    ...
    def write_price_log(self, price, action):
        ...
        log_file = "deltaforce_919_price_log.csv"
        ...
        try:
            ...
            while not self.should_stop:
                try:
                    ...
                    # 主程序逻辑：get_ammo_price(4) -> 延迟0.01 -> esc -> 延迟0.45 (目标价格200-480)
                                    ...
                                    if 200 <= price_int <= 480:
                                        ...
                                        if self.main_delta:
                                            if self.manager.focus_main():
                                                ...
                                                # 循环点击0.8511，0.7994 6次，每次延迟0.05s
                                                for i in range(3):
                                                    if self.should_stop:
                                                        break
                                                    self.main_delta.click_ratio(0.8511, 0.7994)
                                                    self.log_message.emit(f"🖱️ 主端点击 {i+1}/6")
                                                    time.sleep(0.05)
    ...
    def run(self):
        ...
        try:
            self.log_message.emit("🚀 双端满仓919 开始运行...")
