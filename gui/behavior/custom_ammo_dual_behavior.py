...
class CustomAmmoDualBehavior(QThread):
    ...
    def __init__(self, delta_instance, config=None):
        ...
        # 自定义参数配置（从config中获取，如果没有则使用默认值）
        self.target_price = self.config.get('target_price', 480)
        self.min_valid_price = self.config.get('min_valid_price', 200)
        self.ammo_position = self.config.get('ammo_position', 4)
        self.click_count = self.config.get('click_count', 3)
    ...
    def stop_behavior(self):
        ...
        self.log_message.emit("🛑 正在停止自定义子弹双端满仓行为...")
    ...
    def output_recognition_stats(self):
        ...
        self.log_message.emit("📊 识别准确率统计报告 - 自定义版")
        self.log_message.emit("=" * 50)
    ...
    def write_price_log(self, price, action):
        ...
        log_file = "deltaforce_custom_ammo_price_log.csv"
        ...
        try:
            with open(log_file, 'a', newline='', encoding='utf-8') as f:
                ...
                # 如果文件不存在，写入表头
                if not file_exists:
                    writer.writerow(['时间戳', '价格', '行为', '配置'])
                
                # 写入数据（包含当前配置信息）
                config_info = f"目标价格阈值:{self.target_price},最低有效:{self.min_valid_price},子弹位置:{self.ammo_position}"
                writer.writerow([timestamp, price, action, config_info])
    ...
    def initialize_manager(self):
        ...
        try:
            ...
            # 设置日志回调，让DeltaForce实例能够将验证失败信息发送到GUI
            if self.main_delta:
                self.main_delta.set_log_callback(lambda msg: self.log_message.emit(msg))
            if self.aux_delta:
                self.aux_delta.set_log_callback(lambda msg: self.log_message.emit(msg))
            ...
            # 输出自定义配置信息
            self.log_message.emit("🎯 自定义配置参数:")
            self.log_message.emit(f"   目标价格阈值: {self.target_price} (低于此价格即为有效)")
            self.log_message.emit(f"   最低有效价格: {self.min_valid_price} (低于此价格认为是识别错误)")
            self.log_message.emit(f"   子弹位置: {self.ammo_position}")
            self.log_message.emit(f"   主端点击次数: {self.click_count}")
    ...
    def main_logic_worker(self):
        ...
        try:
                    ...
                    if price is not None:
                        ...
                        if self.harmony_mode:
                            ...
                            try:
                                ...
                                # 检查价格是否低于目标价格阈值（即为有效，执行操作）
                                elif price_int < self.target_price:
                                    self.log_message.emit(f"🎯 价格 {price} 低于目标阈值 {self.target_price}，执行特殊操作")
            ...
            self.log_message.emit(f"❌ 主逻辑启动失败: {e}")
        finally:
            # 确保停止click_ammo线程
            self.stop_click_ammo()
    
    def run(self):
        """主运行循环 - 专门执行自定义子弹双端满仓逻辑"""
        self.is_running = True
        self.status_changed.emit("running")
        
        try:
            self.log_message.emit("🚀 自定义子弹双端满仓行为 开始运行...")
            self.log_message.emit("🎛️ 自定义版特性：支持参数配置，灵活适应不同需求")
